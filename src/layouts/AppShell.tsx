...
export const AppShell: React.FC<AppShellProps> = ({ children }) => {
  ...
  const investorNav = [
    { icon: LayoutDashboard, label: 'Терминал', path: '/app/investor' },
    { icon: Wallet, label: 'Активы', path: '/app/investor/portfolio' }
  ];

  const traderNav = [
    { icon: LayoutDashboard, label: 'Терминал', path: '/app/trader' },
    { icon: BookOpen, label: 'Обучение', path: '/app/trader/courses' },
    { icon: Wrench, label: 'Инструменты', path: '/app/trader/tools' }
  ];
      ...
      <div className="w-64 border-r border-white/10 bg-black flex flex-col">
        ...
        <div className="p-4 border-t border-white/10 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-bold">{profile?.full_name || 'Пользователь'}</div>
              <div className="text-xs text-white/40 font-mono uppercase">
                {isInvestor ? 'Инвестор' : isTrader ? 'Трейдер' : 'Пользователь'}
              </div>
            </div>
